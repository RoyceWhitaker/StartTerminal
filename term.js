...
function preCheck(str) {
    ...
    //performs a google search using s [search query] or search [search query]
    if(str == "search"){
        //Do nothing
    }
    else{
        if (str.slice(0,2) == "s " && str.startsWith("search") == false) {
            loadURL("https://google.com/search?q=" + str.slice(1))
            return true
        }
        if (str.startsWith("search ") == true) {
            searchTerm = str.substring( str.indexOf(" ") + 1, str.length );
            loadURL("https://google.com/search?q=" + searchTerm);
            return true;
        }
    }
